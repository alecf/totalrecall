...
Views (SwiftUI)
  ...
  → MemoryRiverView (proportional stacked bar, split at a midline into a
    ...
    left gutter naming the two halves "In RAM" / "Compressed")
...
- **The river's two halves are named in a left gutter** — a static "In RAM" centered in the band and "Compressed" anchored to the midline, in a fixed `Theme.riverAxisLabelWidth` column so the bar's left edge doesn't move with the text. The lower one says "Compressed" rather than "Swapped" even though the quantity it names (`physFootprint - residentSize`) conflates compressed and swapped pages and cannot be split per process — the compressor is the half macOS reaches first, it's the word Activity Monitor uses for the same memory, and `readoutText` and `Theme.memoryCompressed` already use it. That's three places plus the site copy; rename in all of them or none
