...
- **Releasing** (two-phase, PR-gated — nothing reaches `main` or users without a merged PR):
  1. `gh workflow run release.yml` — git-cliff auto-calculates semver from commits (`feat:` → minor, `fix:` → patch, `feat!:` → major), generates the grouped changelog, builds + ad-hoc-signs the `.app` bundle + DMG, EdDSA-signs the DMG, creates a **draft** GitHub Release holding the DMG, opens a `release/vX.Y.Z` PR that adds the new `appcast.xml` entry, and enables auto-merge on it.
  2. The PR merges itself once its checks pass. The merge runs `release-publish.yml`, which flips the draft Release to published (creating the tag at the merge commit); `deploy-site.yml` publishes the updated appcast to GitHub Pages. To abort, disable auto-merge or close the PR — the draft Release stays unpublished.
- **The release PR is opened by a GitHub App** (`RELEASE_APP_ID` / `RELEASE_APP_PRIVATE_KEY`), not `GITHUB_TOKEN`. GitHub starts no workflow run for events created with `GITHUB_TOKEN`, so a PR opened with it gets no CI and its required checks never fill in. Do not "simplify" that step back to the default token — it takes the checks, and therefore auto-merge, with it. Setup steps are in `docs/sparkle-setup.md`
