...
- **Memory River**: proportional stacked bar across all your RAM — apps on the left, an "Other" segment for used memory no app claims, free space on the right, with a readout line beneath the bar that names the hovered segment and its share of total RAM. Segment widths are resident memory, and each band fills amber from the bottom in proportion to how much of that app is compressed or swapped — so a mostly-amber band is an app much larger than it looks
